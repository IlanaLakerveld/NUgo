...
//		gogui.addStone(8, 8, false);
    ...
    public static void main(String[] args) {
        ...
        int lastCol = 0;
        int lastRow = 0;
        final int checkSize = go.gogui.getBoardSize() - 1;
        try {
            while (lastCol <= checkSize) {
                Random random = new Random();
                go.gogui.addStone(lastCol, lastRow, random.nextBoolean());

                if (lastRow < checkSize) {
                    lastRow++;
                } else {
                    lastCol++;
                    lastRow = 0;
